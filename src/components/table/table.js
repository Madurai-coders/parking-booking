...
export default function Table(props) {
  ...
  const [get_mail, setGetMail] = useState({flag:false,mail:'',status:'send',validation:'',describe:''});

  function removeCol(head) {
    var store = [];
    var header = [];
  ...
  function click() {
    ...
    if (array && data_array) {
      ...
      if(response){
      setGetMail({flag:false,mail:'',status:'Send',validation:'',describe:'',sent:'success'})
      setTimeout(() => {
        setGetMail({flag:false,mail:'',status:'Send',validation:'',describe:'',sent:''})
      }, 2000);
    }
    else{
        setGetMail({flag:false,mail:'',status:'Send',validation:'',describe:'',sent:'failed'})
    }
    setTimeout(() => {
        setGetMail({flag:false,mail:'',status:'Send',validation:'',describe:'',sent:''})
      }, 2000);
  ...
  function mailSubmit(){
      console.log(validation_email(get_mail.mail))
       ...
       setGetMail({...get_mail,validation:'Check the mail id you have entered'})
  ...
  return ( 
    <>
    { get_mail.sent=='success' && <div class="alert alert-success" role="alert">
       Mail delivered successful
</div>}
{ get_mail.sent=='failed' && <div class="alert alert-success" role="alert">
        Mail delivered failed
</div>}
    { get_mail.flag &&
    <div class="overlay1" >
         { get_mail.validation && <div class="alert alert-danger" role="alert">
       { get_mail.validation}
</div>}
  ...
  <div class="modal-dialog" >
    ...
    <input type="email" class="form-control mb-1" id="exampleInputEmail1"
     ...
     onChange={(e)=> (setGetMail({...get_mail,validation:''}),
     setGetMail({...get_mail,mail:e.target.value,validation:""}))}
     placeholder="Enter email"/>
    {/* <small id="emailHelp" class="form-text text-muted">We'll never share your email with anyone else.</small> */}
  </div>
      <div class="modal-footer">
        <button type="button" onClick={mailSubmit} class="btn btn-primary btn-sm" 
        disabled={get_mail.status=='Sending'?true:false}>{get_mail.status}</button>
        <button type="button"
         onClick={()=>setGetMail({flag:false,mail:'',status:'send',validation:'',describe:''})}
 class="btn btn-secondary btn-sm" data-dismiss="modal">Close</button>
      </div>
    </div>
  </div>
</div>
      ...
      {data && headers && (<div className="m-1 mb-2">
        <CSVLink
            ...
            props.data.map((data, id) => {
              return (
                <tr key={id} className="booking_report_table_data">
                  ...
                  <td className="data5">{data.data6}</td>
                  <td className="data5">{data.data7}</td>
                  <td className="data5">{data.data8}</td>
                  <td className="data5">{data.data9}</td>
                </tr>
              );
            })}
        </table>
